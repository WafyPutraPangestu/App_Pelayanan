        ...
        <div class="max-w-4xl mx-auto px-4">
            <div class="text-center mb-8" x-data="{ show: false }" x-init="setTimeout(() => show = true, 100)">
                <div x-show="show" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 transform translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0">
                    ...
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">Edit Pengaduan</h1>
                    <p class="text-lg text-gray-600 max-w-2xl mx-auto">
                        Perbarui detail pengaduan Anda jika ada informasi yang perlu diperbaiki atau ditambahkan.
                    </p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden" x-data="formHandler()">
                <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    <h2 class="text-xl font-semibold text-gray-900 flex items-center">
                        Form Pengaduan: <span class="ml-2 font-bold text-blue-600">{{ $pengaduan->nomor_pengaduan }}</span>
                    </h2>
                </div>

                <form action="{{ route('pengaduan.update', $pengaduan->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8" @submit="handleSubmit">
                    @csrf
                    @method('PUT')
                    ...
                    <div x-data="{ focused: false }">
                        <label for="judul" class="block text-sm font-semibold text-gray-900 mb-3">
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-2 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"/></svg>
                                Judul Pengaduan
                        ...
                        <div class="relative">
                            <input type="text" 
                                   ...
                                   placeholder="Contoh: Perbaikan Data pada Surat Domisili"
                                   maxlength="255">
                    ...
                    <div x-data="{ focused: false }">
                        <label for="kategori" class="block text-sm font-semibold text-gray-900 mb-3">
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-2 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                                Kategori Pengaduan
                                <span class="text-red-500 ml-1">*</span>
                            </span>
                        </label>
                        <select id="kategori" 
                            ...
                            <option value="{{ $key }}" @selected(old('kategori', $pengaduan->kategori) == $key)>
                                {{ $key }} - {{ $description }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div x-data="{ focused: false, wordCount: 0 }" x-init="wordCount = formData.isi_pengaduan.length">
                        ...
                        <div class="relative">
                            <textarea id="isi_pengaduan" 
                                      ...
                                      placeholder="Jelaskan detail pengaduan Anda dengan lengkap dan jelas...">{{ old('isi_pengaduan', $pengaduan->isi_pengaduan) }}</textarea>
                    ...
                    <div x-data="{ dragover: false, fileName: '', fileSize: '', handleDrop(e) { this.dragover = false; const files = e.dataTransfer.files; if (files.length > 0) { this.handleFileSelect(files[0]); } }, handleFileSelect(file) { this.fileName = file.name; this.fileSize = this.formatFileSize(file.size); }, formatFileSize(bytes) { if (bytes === 0) return '0 Bytes'; const k = 1024; const sizes = ['Bytes', 'KB', 'MB', 'GB']; const i = Math.floor(Math.log(bytes) / Math.log(k)); return parseFloat((bytes / Math.pow(k, i)).toFixed(2)) + ' ' + sizes[i]; } }">
                        ...
                        @if ($pengaduan->lampiran)
                            <div class="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                                <p class="text-sm text-gray-700">Lampiran saat ini:</p>
                                <a href="{{ Storage::url($pengaduan->lampiran) }}" target="_blank" class="text-blue-600 font-medium hover:underline">{{ basename($pengaduan->lampiran) }}</a>
                                <p class="text-xs text-gray-500 mt-2">Mengunggah file baru akan menggantikan file yang lama.</p>
                            </div>
                        @endif

                        <div class="relative">
                            <div @drop.prevent="handleDrop" @dragover.prevent="dragover = true" @dragleave.prevent="dragover = false" :class="{ 'border-blue-400 bg-blue-50': dragover }" class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 hover:bg-blue-50 transition-all duration-200">
                                <input type="file" id="lampiran" name="lampiran" accept=".jpeg,.jpg,.png,.pdf,.doc,.docx" class="hidden" @change="handleFileSelect($event.target.files[0])">
                                <div x-show="!fileName">
                                    ...
                                    <div class="mt-4"><label for="lampiran" class="cursor-pointer"><span class="mt-2 block text-sm font-medium text-gray-900">Drop files here or <span class="text-blue-600 hover:text-blue-700">browse</span></span></label><p class="mt-2 text-xs text-gray-500">PNG, JPG, PDF, DOC, DOCX up to 2MB</p></div>
                                </div>
                                <div x-show="fileName" class="flex items-center justify-center">
                                    <div class="flex items-center bg-white rounded-lg p-3 shadow-sm border">
                                        <svg class="w-8 h-8 text-blue-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                        <div class="flex-1 min-w-0"><p class="text-sm font-medium text-gray-900 truncate" x-text="fileName"></p><p class="text-xs text-gray-500" x-text="fileSize"></p></div>
                                        <button type="button" @click="fileName = ''; fileSize = ''; document.getElementById('lampiran').value = ''" class="ml-3 text-red-500 hover:text-red-700"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        @error('lampiran')
                        <div class="mt-2 flex items-center text-red-600 text-sm"><svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-100">
                        <button type="submit" 
                                ...
                                class="flex-1 inline-flex justify-center items-center px-6 py-3 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-semibold rounded-lg hover:from-yellow-600 hover:to-orange-600 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl disabled:hover:scale-100 disabled:hover:shadow-lg">
                            <svg x-show="!submitting" class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
                            <svg x-show="submitting" class="w-5 h-5 mr-2 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                            <span x-text="submitting ? 'Memperbarui...' : 'Update Pengaduan'"></span>
                        </button>
