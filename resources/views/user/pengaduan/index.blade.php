        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div x-show="viewMode === 'grid'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div x-show="showPengaduan({{ json_encode([
                        'id' => $pengaduan->id,
                    ...
                    ]) }})" class="bg-white rounded-2xl shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 overflow-hidden border border-gray-100">
                        <div class="p-6">
                    ...
                    <div x-show="hasSearchOrFilter && !hasVisibleResults" class="col-span-full">
                        <div class="text-center py-12 bg-white rounded-2xl shadow-xl">
                            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                            </svg>
                            <h3 class="mt-4 text-lg font-medium text-gray-900">Tidak ada pengaduan ditemukan</h3>
                            <p class="mt-2 text-gray-500">Coba ubah kata kunci pencarian atau filter status.</p>
                            <div class="mt-6">
                                <button @click="searchQuery = ''; selectedStatus = ''" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white font-medium rounded-xl hover:bg-gray-700 transition-all duration-200">
                                    Reset Filter
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($pengaduans as $pengaduan)
                                <tr x-show="showPengaduan({{ json_encode([
                                    'id' => $pengaduan->id,
                                    'nomor_pengaduan' => $pengaduan->nomor_pengaduan,
                                    'judul' => $pengaduan->judul,
                                    'isi_pengaduan' => $pengaduan->isi_pengaduan,
                                ...
                                <!-- Empty state for table search results -->
                                <tr x-show="hasSearchOrFilter && !hasVisibleResults">
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                        </svg>
        ...
        function pengaduanData() {
            return {
                ...
                get hasVisibleResults() {
                    const pengaduanElements = document.querySelectorAll('[x-show*="showPengaduan"]');
                    return Array.from(pengaduanElements).some(el => el.style.display !== 'none');
                },
                
                showPengaduan(pengaduan) {
                    let show = true;
                    
                    // Filter by search query
                    if (this.searchQuery.trim() !== '') {
                        const query = this.searchQuery.toLowerCase().trim();
                        show = pengaduan.judul.toLowerCase().includes(query) ||
                               pengaduan.isi_pengaduan.toLowerCase().includes(query) ||
                               pengaduan.nomor_pengaduan.toLowerCase().includes(query) ||
                               pengaduan.user_name.toLowerCase().includes(query) ||
                               pengaduan.kategori.toLowerCase().includes(query);
                    }
                    
                    // Filter by status
                    if (show && this.selectedStatus !== '') {
                        show = pengaduan.status === this.selectedStatus;
                    }
                ...
                updateVisibility() {
                    // Force update of hasVisibleResults
                    this.$nextTick(() => {
                        this.hasVisibleResults;
                    });
                }
            }
        }
