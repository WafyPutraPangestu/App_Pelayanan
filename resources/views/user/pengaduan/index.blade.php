<x-layout>
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50" x-data="{ viewMode: 'grid' }">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">Kelola dan pantau semua pengaduan Anda</p>
                        </div>
                    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <form action="{{ route('pengaduan.index') }}" method="GET">
                    <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                        <div class="flex flex-col sm:flex-row gap-4 flex-1 items-center">
                            ...
                            <button type="submit" class="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition">Filter</button>
                        </div>
                        
                        <div class="flex gap-2">
                             <button type="button" @click="viewMode = 'grid'" :class="viewMode === 'grid' ? 'bg-blue-100 text-blue-600' : 'text-gray-400 hover:text-gray-600'" class="p-2 rounded-lg transition-all duration-200">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                            </button>
                            <button type="button" @click="viewMode = 'list'" :class="viewMode === 'list' ? 'bg-blue-100 text-blue-600' : 'text-gray-400 hover:text-gray-600'" class="p-2 rounded-lg transition-all duration-200">
            ...
            <div x-show="viewMode === 'grid'" x-transition>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col">
                        ...
                        <div class="p-6 bg-gray-50 border-t">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                @if($pengaduan->status == 'baru')
                                    <a href="{{ route('pengaduan.edit', $pengaduan) }}" class="p-2 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition" title="Edit"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                                @endif
                            </div>
                        </div>
                    </div>
                    @empty
                    <div class="col-span-full text-center py-12 bg-white rounded-2xl shadow-xl">
                        <h3 class="mt-4 text-lg font-medium text-gray-900">Tidak ada pengaduan ditemukan</h3>
                        <p class="mt-2 text-gray-500">Tidak ada data yang cocok dengan filter Anda.</p>
                    </div>
                    @endforelse
                </div>
            </div>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4"><div class="flex items-center space-x-4">
                                        <a href="{{ route('pengaduan.show', $pengaduan) }}" class="text-blue-600 hover:text-blue-900 font-medium text-sm">Detail</a>
                                        @if($pengaduan->status == 'baru')
                                            <a href="{{ route('pengaduan.edit', $pengaduan) }}" class="text-yellow-600 hover:text-yellow-900 font-medium text-sm">Edit</a>
                                            <form action="{{ route('pengaduan.destroy', $pengaduan) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus pengaduan ini?')">
                                                @csrf @method('DELETE')
                                ...
                                @empty
                                <tr><td colspan="6" class="px-6 py-12 text-center"><h3 class="text-lg font-medium text-gray-900">Tidak ada pengaduan ditemukan</h3></td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
