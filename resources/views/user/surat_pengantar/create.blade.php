<x-layout>
    <h1 class="text-2xl font-bold text-gray-800 mb-6 text-center">Formulir Surat Pengantar</h1>

    <div class="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg border">
        <form action="{{ route('surat_pengantar.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="md:col-span-2">
                 ...
                 <div class="md:col-span-2">
                    ...
                    <textarea id="maksud_dan_tujuan" name="maksud_dan_tujuan" rows="3" class="w-full form-input" placeholder="Contoh: Untuk mengurus SKCK di Polsek">{{ old('maksud_dan_tujuan') }}</textarea>
                    @error('maksud_dan_tujuan') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
            </div>
